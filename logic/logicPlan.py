...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    return findModel(implication) is False
    "*** END YOUR CODE HERE ***"
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    clauses = []
    for combo in combinations(literals, 2):
        clauses.append(disjoin([~combo[0], ~combo[1]]))
...
def pacphysicsAxioms(
    ...
    pacphysics_sentences += (
        [] if sensorModel is None else [sensorModel(t, non_outer_wall_coords)]
    )
    pacphysics_sentences += (
...
def checkLocationSatisfiability(
    ...
    "*** BEGIN YOUR CODE HERE ***"
    KB += [
        pacphysicsAxioms(0, all_coords, non_outer_wall_coords, walls_grid),
    ...
    KB += [
        PropSymbolExpr(pacman_str, x0, y0, time=0),
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        for x, y in food:
            ...
            B = PropSymbolExpr(pacman_str, x, y, time=t) & PropSymbolExpr(
                food_str, x, y, time=t
            )
...
def localization(problem, agent) -> Generator:
    ...
    KB = []  # Initialize kb with wall information
    ...
    for t in range(agent.num_timesteps):
        ...
        for x, y in non_outer_wall_coords:
            loc_expr = PropSymbolExpr(pacman_str, x, y, time=t)  # Check if Pacman can be at (x, y) using model finding
            if entails(conjoin(KB), loc_expr):
                possible_locations.append((x, y))
                KB.append(loc_expr)
            elif entails(conjoin(KB), ~loc_expr):
                KB.append(~loc_expr)
            else:
                possible_locations.append((x, y))
...
def mapping(problem, agent) -> Generator:
    ...
    known_map[pac_x_0][pac_y_0] = 0  #  mark  Pacman's initial position as open space 
    for t in range(agent.num_timesteps): #  pacphysics, actions, and percepts to the  KB 
        KB.append(pacphysicsAxioms(t, all_coords, non_outer_wall_coords, known_map, sensorAxioms, allLegalSuccessorAxioms)) 
        KB.append(PropSymbolExpr(agent.actions[t], time=t)) # Update KB  based on the sensor readings 
        percepts = agent.getPercepts()
        temp =  [] 
        KB.append(fourBitPerceptRules(t, percepts)) # Deduce and update known_map based on the KB 
        for x, y in non_outer_wall_coords: 
            wall_expr = PropSymbolExpr(wall_str, x, y) 
            no_wall_expr = ~wall_expr # If it can be proven there's a wall 
            if entails(conjoin(KB), wall_expr) and not entails(conjoin(KB),no_wall_expr):
                known_map[x][y] = 1 # If it can be proven there's no wall 
                temp.append(wall_expr)
            elif entails(conjoin(KB), no_wall_expr) and not entails(conjoin(KB), wall_expr): 
                known_map[x][y] = 0 
                temp.append(no_wall_expr)
                
        KB.append(conjoin(temp)) 
        agent.moveToNextState(agent.actions[t]) 
...
def slam(problem, agent) -> Generator:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    # possible_locations = [(pac_x_0, pac_y_0)] 
    KB.append(PropSymbolExpr(pacman_str, pac_x_0, pac_y_0, time=0)) 
    KB.append(~PropSymbolExpr(wall_str, pac_x_0, pac_y_0))
    #initialize the map

    known_map[pac_x_0][pac_y_0] = 0
   # why not time = t ? ask TA
    #same as 7
  
    for t in range(agent.num_timesteps): # KB  with physics, actions, and percepts 
        ...
        for x, y in non_outer_wall_coords: 
            ...
            if entails(conjoin(KB), wall_expr):
                known_map[x][y] = 1 # If it can be proven there's no wall 
                KB.append(wall_expr)
            if entails(conjoin(KB), no_wall_expr): #and not entails(conjoin(KB), wall_expr): 
                known_map[x][y] = 0 
                KB.append(no_wall_expr)
        possible_locations = []     
        
        for (x, y) in non_outer_wall_coords:
            vv = PropSymbolExpr(pacman_str, x, y, time=t)
            if(findModel(conjoin(conjoin(KB),vv )) != False):
                possible_locations.append((x,y))
